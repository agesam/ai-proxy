...
function buildSystemPrompt(externalData, externalmaterialData, conversationHistory) {
    ...
    // 定義家長模式關鍵詞 (優先級高，專注於行政/課程資訊)
    const PARENT_KEYWORDS = ['課程', '早慧', '分校', '報名', '學費', '時間表', '查詢', '行政', '作文', '中文'];
    // 定義學生模式關鍵詞 (專注於教材/知識內容) - 學生模式是預設，此列表用於輔助判斷
    const STUDENT_KEYWORDS = ['動畫', '動物', '故事', '小知識', '成語', '睇'];
    
    // 獲取最新的用戶訊息並轉換為小寫，以便進行不區分大小寫的關鍵詞檢查
    ...
    // 檢查家長模式關鍵詞：只要包含任何一個家長關鍵詞，即切換為家長模式
    const isParentMode = PARENT_KEYWORDS.some(keyword => userMessage.includes(keyword));
    ...
    // 【2. 通用規則和安全限制】 (從您原有的 Prompt 中提取，確保一致性)
...
在提供完有關的主要資訊後，你【必須】提出一個單選題（不多於4個選項），用以引導使用者進一步探索相關主題。
** 你的提問【絕對不可以】是開放式問題 **
---
**【🚨格式強制規範：絕對不可變動🚨】**
**請將以下標籤視為**不可協商的程式碼標籤**。**
...
` + COMMON_RULES_AND_SAFETY;
    ...
    // 【5. 組合最終 Prompt 與資料】
    let finalPrompt = isParentMode ? PARENT_PROMPT_TEMPLATE : STUDENT_PROMPT_TEMPLATE;

    let combinedContext = finalPrompt + 
        "\n\n【早慧資料】(JSON):\n" + JSON.stringify(externalData, null, 2) +
        "\n\n【動畫教材資料】(JSON):\n" + JSON.stringify(externalmaterialData, null, 2);
